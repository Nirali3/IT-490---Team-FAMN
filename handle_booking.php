...
//}
	...
	//$user_id = $_SESSION['user_id'];
        ...
        $stmt->bind_param("isssssdssss", $user_id, $airline, $departureAirport, $destinationAirport, $departureDate, $arrivalDate, $price, $card_number, $cardholder_name, $expiration_date, $cvc);
        $stmt->execute();
        ...
        foreach ($first_names as $index => $first_name) {
            $stmt->bind_param("isssssi", $booking_id, $first_names[$index], $last_names[$index], $dobs[$index], $cabin_classes[$index], $age_groups[$index], $user_id);
            $stmt->execute();
        }
